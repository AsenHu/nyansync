...
impl FileMeta {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        ...
        let extension = match bytes[28] {
            ...
            2 => FileExt::Webp,
            3 => FileExt::Png,
            ext => return Err(Error::InvalidFileExtNum(ext).into()),
        };
        let file = Self {
            size,
            hash,
    ...
    pub fn from_str(s: &str) -> Result<Self, Error> {
        ...
        if parts.len() != 5 {
            return Err(Error::InvalidStringFormat.into());
        }
        let mut hash = [0u8; 20];
        ...
        let extension = match parts[4].to_lowercase().as_str() {
            ...
            "wbp" => FileExt::Webp,
            "png" => FileExt::Png,
            s => return Err(Error::InvalidFileExtName(s.to_string()).into()),
        };
        let file = Self {
            size,
            hash,
    ...
    fn check(&self) -> Result<(), Error> {
        ...
        if self.width < 100 || self.width > 20000 {
            return Err(Error::InvalidWidth(self.width).into());
        };
        // 检查高度
        if self.height < 100 || self.height > 20000 {
            return Err(Error::InvalidHeight(self.height).into());
        };
        // 宽度高度之和应该大于 250（不包含）
        let sum = self.width + self.height;
        if sum <= 250 {
            return Err(Error::InsufficientResolution(sum).into());
        };

        // 检查文件大小
        if self.size == 0 {
            return Err(Error::InvalidFileSizeZero.into());
        };
        // 检查大小是否符合扩展名
        // GIF 不超过 10 MiB
        // JPG 不超过 20 MiB
        ...
        if self.size > max_size {
            return Err(Error::SizeExceedsLimitForExtension(self.size, max_size).into());
        }
...
impl File {
    pub fn build(meta: FileMeta, content: Vec<u8>) -> Result<Self, Error> {
        if meta.size as usize != content.len() {
            return Err(Error::FileSizeMismatch.into());
        }
        #[cfg(feature = "checksum")]
        {
            let computed_hash = Sha1::digest(&content);
            if computed_hash[..] != meta.hash {
                return Err(Error::FileHashMismatch.into());
            }
        }
